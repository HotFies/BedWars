...
public class v1_18_R2 implements VersionInterface {
	...
	public Fireball launchFireball(Player p) {
		...
		CustomFireball fireball = new CustomFireball(cp.s, cp, direction.getX(), direction.getY(), direction.getZ());
		fireball.projectileSource = p;
		fireball.a(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
		cp.cA().b(fireball);
	...
	public void isArmorVisible(boolean value, Player p, List<Player> players) {
		...
		for (Player all : players) {
			((CraftPlayer) all).getHandle().b.a(new PacketPlayOutEntityEquipment(ep.ae(), equipment));
		}
	}
	...
	public ArmorStand createGenerator(Location l) {
		...
		generator.a(new BlockPosition(l.getBlockX(), l.getBlockY(), l.getBlockZ()), 0, 0);
